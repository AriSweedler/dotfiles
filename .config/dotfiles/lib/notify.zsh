...
note() {
  local ts
  strftime -s ts '%Y-%m-%d %H:%M:%S' "${ARI_DOTFILES_NOW}"
...
# launchd (ARI_DOTFILES_INVOKED_BY) and without --no-notify; interactive runs get a log line.
...
hud() {
  ...
  if [[ -z "${ARI_DOTFILES_INVOKED_BY:-}" || "${ARI_DOTFILES_NO_NOTIFY:-0}" == 1 ]]; then
  ...
  if [[ -n "${ARI_DOTFILES_NOTIFIER}" ]] && command -v "${ARI_DOTFILES_NOTIFIER}" >/dev/null 2>&1; then
    ...
    if [[ "${open_url}" == file://* ]]; then
      ...
      args+=(-execute "${(q)ARI_DOTFILES_EDIT_WINDOW} -n ${CLI_NAME} ${(q)report_file}")
    ...
    if (( seconds > 0 )); then
      sleep "${seconds}"
      "${ARI_DOTFILES_NOTIFIER}" -remove "${CLI_NAME}" >/dev/null || true
    fi
    return 0
  fi
