...
Draft dialect (/ari-hemingway--format-gdoc, Asides): a marker `([aside n ℹ️](#aside-n))` in the body, a
section `# Aside n: <title>` after `# Useful links`, and, anywhere else, references `[<title>](#aside-n)`
whose text is exactly the aside's title. Two modes:
...
section body with an empty paragraph between blocks, as the import lays out the main tab / `← Return to
main article` (italic, rule above). The Docs API has no horizontal-rule or bookmark request: the rules
are paragraph borders, and the return link lands on the marker's nearest preceding heading in the first
tab. With --bookmarks an Apps Script function (bin/gas/Asides.gs) adds a bookmark at each marker first
and the return link lands there instead.
...
BOOKMARK_FUNCTION = "addMarkerBookmarks"
...
def check_reference(n, text, sections, where):
    """A reference reads exactly like the tab it points at, so it stays searchable by tab name."""
...
def link_runs(link_text, url):
    ...
    since it must read exactly like the tab title."""
...
def batch_update(doc, requests, revision, dry_run, what):
    if not requests:
        return {}
    params = json.dumps({"documentId": doc})
    ...
    if dry_run:
        ...
        print(body)  # stdout is data: the planned batch
        return {}
    return gws_json(["docs", "documents", "batchUpdate", "--params", params, "--json", body], what=what)
...
def tab_by_id(doc_json, tab_id):
    ...
    die(f"tab missing after rebuild | tab='{tab_id}'")
...
# --- Bookmarks through Apps Script (optional) ---


def add_bookmarks(doc, max_n, script_id):
    """Run addMarkerBookmarks in the Apps Script project; returns {n: bookmarkId}."""
    params = json.dumps({"scriptId": script_id})
    body = json.dumps({"function": BOOKMARK_FUNCTION, "parameters": [doc, max_n], "devMode": True})
    reply = gws_json(["script", "scripts", "run", "--params", params, "--json", body], what="scripts.run")
    if "error" in reply:
        die(f"scripts.run failed | error='{json.dumps(reply['error'])[:300]}'")
    result = reply.get("response", {}).get("result") or {}
    log_info(f"bookmarks added | count='{len(result)}' script='{script_id}'")
    return {int(n): bookmark_id for n, bookmark_id in result.items()}
...
def return_link(return_target):
    return {"link": return_target, "italic": True}
...
def build_fill_requests(tab_id, heading, blocks, return_target, tab_ids):
    ...
    styles.append(text_style(tab_id, start, end - 1, return_link(return_target), "link,italic"))
    ...
    styles.append(text_style(tab_id, start, end, return_link(return_target), "link,italic"))
...
def choose_return_target(n, tab, paragraph_element, bookmarks):
    if n in bookmarks:
        return {"bookmark": {"id": bookmarks[n], "tabId": FIRST_TAB}}, "bookmark"
    return {"heading": {"id": nearest_heading_id(tab, paragraph_element), "tabId": FIRST_TAB}}, "heading"
...
def run_rebuild(args):
    ...
    bookmarks = {}
    if args.bookmarks and not args.dry_run:
        bookmarks = add_bookmarks(doc, max(asides), args.script_id)
    if not args.dry_run:
        doc_json = fetch_doc(doc)
    ...
    for n in sorted(asides):
        ...
        return_target, return_kind = choose_return_target(n, tab, paragraph_element, bookmarks)
        requests, n_chips = build_fill_requests(tab_id, tab_title(n, asides[n]["title"]), asides[n]["blocks"], return_target, tab_ids)
        ...
        summary.append({"n": n, "title": asides[n]["title"], "tab": tab_id, "return": return_kind, "chips": n_chips, "body_references": len(references), "marker_range": [marker_run["startIndex"], marker_run["endIndex"]]})
        log_info(f"aside tab rebuilt | n='{n}' title='{asides[n]['title']}' tab='{tab_id}' return='{return_kind}' chips='{n_chips}' body_references='{len(references)}' marker_range='{marker_run['startIndex']}-{marker_run['endIndex']}'")
...
def parse_args():
    ...
    parser.add_argument("--bookmarks", action="store_true", help="--rebuild: add a bookmark at each marker through Apps Script (needs --script-id)")
    parser.add_argument("--script-id", dest="script_id", help="--rebuild --bookmarks: Apps Script deployment or script id")
...
def main():
    ...
    if args.bookmarks and not args.script_id:
        die("--bookmarks needs --script-id | hint='see Asides setup in the share-gdoc SKILL.md'")
    if args.split:
        run_split(args)
        return
    check_prerequisites()
