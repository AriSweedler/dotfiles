...
description: Convert a Google Doc — or one tab of it (?tab=<id>) — into markdown, preserving headings, lists, tables, links, smart chips (rich-link/person/date), and inline images, via a deterministic Python converter. Default output reads well; `--publish` emits the /ari-hemingway--format-gdoc dialect that /ari-hemingway--share-gdoc takes unchanged. Use when you need a Google Doc as markdown to read or diff, or to update an existing Doc through the hemingway pipeline. For a folder of docs, use /ari-gsw-read.
---

# Google Doc to Markdown

Turn a Google Doc (or one of its tabs) into a local markdown file you Read, or — with `--publish` — into the draft `/ari-hemingway--share-gdoc` republishes unchanged. The conversion lives in a checked-in Python script, so the same document always yields the same markdown; `--json-file` re-converts a saved fetch with no network call.
...
Use this for ANY "give me / read / diff this Google Doc (or this tab) as markdown" request, and — with `--publish` — whenever an existing Doc is about to be edited and republished through `/ari-hemingway--share-gdoc`. Do NOT hand-roll the conversion — no `gws docs … | jq`, no ad-hoc Docs-JSON parsing, no browser copy-paste. The checked-in converter is the only supported path; anything else drops smart chips, tables, and inline images.
...
1. Run the converter:
   ```zsh
   zsh $HOME/.claude/skills/ari-gsw-doc-to-md/bin/doc-to-md.zsh "<source>" [--tab ID] [--out PATH] [--stdout] [--force] [--publish]
   ```
...
- `--publish`: emit the `/ari-hemingway--format-gdoc` dialect instead of the read dialect (see the third column of the output contract). Use it to fetch a Doc you will edit and hand back to `/ari-hemingway--share-gdoc`; the read dialect needs three hand-fixes first (title line, raw Drive URLs, linked images), which is exactly the drift this flag removes.
...
| Docs construct | Markdown (default) | With `--publish` |
|---|---|---|
| Doc title (metadata) | dropped | plain-text first line, then a blank line |
| TITLE / HEADING_1 … HEADING_6 | `#` … `######` | same |
| bold / italic / monospace font | `**` / `*` / `` ` `` | same |
| bulleted vs ordered list (by glyph) | `-` vs `1.`, indented 2 spaces per nesting level | same |
| link | `[text](url)` (adjacent same-URL runs coalesced into one link) | same, except a `docs.google.com` / `drive.google.com` URL is emitted raw (anchor text dropped, with a WARN) so the publish step can chip it |
| rich-link (doc/drive/url) chip | `[title](url)` | raw URL for a Drive target (the chip shows the title); other targets unchanged |
| person chip | `name <email>` | same |
| date chip | display text (e.g. `Jul 8, 2026`) | same |
| table | GitHub pipe table, `\|` escaped in cells | same |
| inline image | `![alt](contentUri)` | linked to a mermaid.live pako source: `[![alt](mermaid.ink/img/base64:…?width=620)](live_url)`, the ink URL derived from the same payload; linked elsewhere: `[![alt](contentUri)](link)` with a WARN; unlinked: as default |
...
Inline-image `contentUri` values are Google URLs that expire (minutes-to-hours). They render but will 404 later — re-run the converter for fresh URLs. `--publish` sidesteps this for diagrams by rendering from mermaid.ink, which is why a Doc image should always link to its mermaid.live source.
