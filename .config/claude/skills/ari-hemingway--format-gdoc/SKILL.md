...
Never link a Claude skill in a Doc: no `github.com/AriSweedler/dotfiles/...` URL anywhere, no `## Code and PRs` block that lists the toolchain. A doc is for readers of its topic; if a producing skill wants provenance, it is one plain-text sentence naming the skills, and `# Useful links` holds topic sources only.

- GOOD: `This article was generated by the ari-hemingway pipeline: ari-hemingway--structure-scaffold shaped it and ari-hemingway--share-gdoc published it.`
...
- **Final line MUST be `🤖🌸 Generated with Claude Code`** — on its own, no heading.
