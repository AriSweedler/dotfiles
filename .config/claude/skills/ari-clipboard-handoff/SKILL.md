...
- The widget is not in this skill. `dotfiles raycast link` (`~/.config/bin/dotfiles`, logic in `~/.config/zsh/plugins/raycast_link.zsh`) renders a Raycast action as a link with its Karabiner binding. The bindings live in karabiner.ts's TypeScript tables (`~/.config/karabiner/karabiner.ts/src/modes/*.ts`, `src/raycast_shortcuts.ts`), which bake compiles into Karabiner; the widget reads them through the same generator, and `src/raycast_bindings.json` is bake's generated artifact for reading, not an input. `dotfiles raycast link check` says whether every binding is compiled and on Raycast's allow-list; to change a binding, edit the table and run bake.
...
```zsh
dotfiles raycast link clipboard-history
```
...
**Widget.** The dotfiles install the entrypoint and the binding; `dotfiles raycast link check` must print `OK clipboard-history ✦4 allowed` (run `bake` under `~/.config/karabiner/bin` if it says MISSING or NOT-ALLOWED). Karabiner owns every key bound in those tables, so Raycast's own hotkey settings stay empty for them: if Raycast still holds a hotkey for a command bound here, clear it there. That step is Ari's.
