...
- The widget is not in this skill. `raycast-link` (`~/.config/bin/raycast-link`, logic in `~/.config/zsh/plugins/raycast_link.zsh`) renders a Raycast action as a link with its Karabiner binding, from `~/.config/karabiner/karabiner.ts/src/raycast_bindings.json`, the same file karabiner.ts compiles into the binding. `raycast-link --check` says whether every binding is compiled; `raycast-link --set <alias> <chord>` changes one and rebakes.
...
**Widget.** The dotfiles install `raycast-link` and its binding; `raycast-link --check` must print `OK clipboard-history ✦4` (run `bake` under `~/.config/karabiner/bin` if it says MISSING). Then, in Raycast, clear Clipboard History's native hotkey (Settings → Extensions → Clipboard History → Hotkey) so ✦4 is not bound twice: Karabiner owns it. That step is Ari's.
