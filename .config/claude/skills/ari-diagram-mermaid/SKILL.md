...
- **Default output is `markdown_link`.** Override `MERMAID_FORMAT` only when the user explicitly asks for another form (e.g. a GitHub-native fenced block).
...
3. **Bake**: `$HOME/.claude/skills/ari-diagram-mermaid/bin/bake <diagram_file>`
...
4. **Present** to user:
   ...
   - For Google Docs: paste the markdown link, or Insert > Image > By URL.
...
| ------------------------ | ----------------------------------------------------------- |
| `markdown_link` (default) | Clickable markdown image link (`[![](img)](edit)`) for Docs |
| `ink_url`                 | Direct mermaid.ink PNG image URL                            |
| `live_url`                | Editable mermaid.live URL                                   |
