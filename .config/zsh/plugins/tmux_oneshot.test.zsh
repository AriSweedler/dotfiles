...
# assembly, autodismiss. Only runs when OTTO_TEST__ZSH_PLUGINS_TMUX_ONESHOT=true
...
# Each _fzf call consumes one line of the picks file: "ESC" → rc 130, else
# grep -E the menu on stdin (multi-line matches emulate tab-multiselect).
...
# ---------------------------------------------------------------------------
# Menu rendering: name = menu.name // cmd, capped column, no separator token
# ---------------------------------------------------------------------------
# Entry 7's name is 49 chars → capped at 40, so the column width is 40 and
# names pad to 42 (width + 2) before the text.
_t "name width caps at TMUX_ONESHOT_NAME_MAX" "40" "$(tmux_oneshot::_name_width)"
_t "menuitem plain: cmd is the name, no body, no padding" \
  "echo plain" "$(tmux_oneshot::key::menuitem 1)"
_t "menuitem unnamed: body excludes cmd" \
  "${(r:42:):-echo}…  # greeter @ /private/tmp" "$(tmux_oneshot::key::menuitem 2)"
_t "menuitem named: menu.name + menu.text" \
  "${(r:42:):-My Name}searchable words" "$(tmux_oneshot::key::menuitem "My Name")"
_t "menuitem text-only: cmd as name, text overrides body" \
  "${(r:42:):-echo text-only}just text" "$(tmux_oneshot::key::menuitem 6)"
local _long_name="$(tmux_oneshot::key::menuitem 7)"
_t "long cmd truncates with ellipsis, never wraps" \
  "echo aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa…" "${_long_name}"
_t "no menu line contains the legacy ' | ' separator" "" \
  "$(for k in ${(f)"$(tmux_oneshot::action::list)"}; do tmux_oneshot::key::menuitem "${k}"; done | grep -F ' | ')"

# ---------------------------------------------------------------------------
# Key recovery: menuitem::key round-trips every key; unknown keys fail clean
# ---------------------------------------------------------------------------
local _k _roundtrip=ok
for _k in ${(f)"$(tmux_oneshot::action::list)"}; do
  [[ "$(tmux_oneshot::menuitem::key "$(tmux_oneshot::key::menuitem "${_k}")")" == "${_k}" ]] || _roundtrip="broke-on-${_k}"
done
_t "menuitem::key round-trips every key" "ok" "${_roundtrip}"
...
My Name
6
7" "$(tmux_oneshot::action::list)"

local _err_out _err_rc
_err_out="$(tmux_oneshot::key::value bogus 2>&1)"; _err_rc=$?
...
# The fzf preview path: fzfdb --preview::menuitem dispatches through the
# client's own menuitem::key (this was the 'Invalid JSON text passed to
# --argjson' bug vector).
_t "preview::menuitem resolves a rendered line to its entry JSON" "echo named-ran" \
  "$(fzfdb --fzfdb_name tmux_oneshot --fzfdb_dir "${_db:h}" -- --preview::menuitem "$(tmux_oneshot::key::menuitem "My Name")" | jq -r '.cmd')"
...
# ---------------------------------------------------------------------------
# handle_selection end-to-end: cwd, eval, hold-vs-autodismiss
# ---------------------------------------------------------------------------
_set_picks "ESC" "assembled"
_t "handle_selection e2e (cwd + eval)" "assembled-in-/private/tmp" \
  "$(tmux_oneshot::key::_handle_selection 2 2>/dev/null | grep assembled-in)"
_t "default holds until esc" "HELD" \
  "$(tmux_oneshot::key::_handle_selection 1 2>/dev/null | grep HELD)"
_t "autodismiss skips hold" "" \
  "$(tmux_oneshot::key::_handle_selection 3 2>/dev/null | grep HELD)"
