...
names=(brew brew_pkgs brew_drift dotfiles_repo local_dotfiles_repo bob_neovim claude terminal_nerdfont karabiner claude_notifications git_health weekly_verify)
listed=0
for s in "${names[@]}"; do
...
assert_json "steps|length == 12" "${f}" '.steps|length' 12
