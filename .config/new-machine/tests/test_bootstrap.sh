...
assert_eq "fetch url is DOTFILES_REMOTE" "${DOTFILES_REMOTE}" "$(df_git remote get-url origin)"
...
assert_eq "df hooksPath set by new-machine" "${NEW_MACHINE_DF_HOOKS}" "$(df_git config --local --get core.hooksPath)"
...
assert_file "clone kept for the re-run" "${NEW_MACHINE_DF_GIT_DIR}/HEAD"
assert_no_file "new-machine never ran" "${NEW_MACHINE_STATE_DIR}"
rm -f "${HOME}/.zshenv"
bump_now_secs 60
bs --json
